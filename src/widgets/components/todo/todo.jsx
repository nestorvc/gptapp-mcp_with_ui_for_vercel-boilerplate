...
function CircleCheckbox({ checked, onToggle, label }) {
  const handleClick = (e) => {
    console.log('[CircleCheckbox] Clicked, checked:', checked, 'label:', label);
    e.stopPropagation();
    console.log('[CircleCheckbox] Calling onToggle...');
    onToggle();
    console.log('[CircleCheckbox] onToggle called');
  };
    ...
    <div
      ...
      onKeyDown={(e) => {
        if (e.key === " " || e.key === "Enter") {
          e.preventDefault();
          console.log('[CircleCheckbox] Keyboard toggle, checked:', checked);
          onToggle();
        }
      }}
      onClick={handleClick}
...
function BaseCard({ children }) {
  return (
    <MotionCard className="absolute top-0 left-0 w-full h-full bg-gray-50/100 overflow-hidden">
      {children}
    </MotionCard>
  );
}
...
export function App() {
  ...
  const openList = (r, index) => {
    console.log('[Todo App] openList called, index:', index);
    ...
    setData((prev) => {
      ...
      console.log('[Todo App] Opened list at index:', index);
      return { lists };
    });
  };
  ...
  const addTodo = () => {
    console.log('[Todo App] addTodo called, currentList:', currentList);
    if (currentList == null) {
      console.warn('[Todo App] addTodo: currentList is null, cannot add todo');
      return;
    }
    const newId = uid();
    console.log('[Todo App] Adding new todo with ID:', newId);
    ...
    setData((prev) => {
      ...
      lists[listIdx] = list;
      console.log('[Todo App] Updated data with new todo, total todos:', list.todos.length);
      return { lists };
    });
  };

  const deleteItemById = (id) => {
    console.log('[Todo App] deleteItemById called with ID:', id);
    if (currentList == null) {
      console.warn('[Todo App] deleteItemById: currentList is null, cannot delete');
      return;
    }
    setData((prev) => {
      const lists = prev.lists.slice();
      const listIdx = currentTodoList;
      const list = { ...lists[listIdx] };
      const beforeCount = list.todos.length;
      list.todos = list.todos.filter((t) => t.id !== id);
      const afterCount = list.todos.length;
      console.log('[Todo App] Deleted todo, count:', beforeCount, '->', afterCount);
      lists[listIdx] = list;
      return { lists };
    });
  };
  ...
  const updateItemById = (id, val) => {
    console.log('[Todo App] updateItemById called, ID:', id, 'Updates:', val);
    if (currentList == null) {
      console.warn('[Todo App] updateItemById: currentList is null, cannot update');
      return;
    }
    setData((prev) => {
      const lists = prev.lists.slice();
      ...
      lists[listIdx] = list;
      console.log('[Todo App] Updated todo item');
      return { lists };
    });
  };
  ...
  return (
    <div className="antialiased">
      <div
        ...
        style={{height: `${MAX_CARD_HEIGHT_REM}rem` }}
      >
        <BaseCard>
              ...
              <div className="p-5">
                ...
                {process.env.NODE_ENV === 'development' && (
                  <div className="mt-2 text-xs text-gray-500 space-y-1">
                    <div>Total Todos: {data.lists.reduce((sum, list) => sum + list.todos.length, 0)}</div>
